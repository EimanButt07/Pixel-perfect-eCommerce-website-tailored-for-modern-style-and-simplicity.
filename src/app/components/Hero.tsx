...
const AnimatedNumberCounter = ({ from, to, duration }: { from: number; to: number; duration: number }) => {
  const [count, setCount] = useState(from);

  useEffect(() => {
    if (count >= to) return;

    const incrementStep = Math.ceil((to - from) / (duration * 60));
    const interval = setInterval(() => {
      setCount((prevCount) => {
        if (prevCount + incrementStep >= to) {
      ...
      <div className="md:max-w-frame mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2">
        ...
        <section className="max-w-frame px-4">
          <motion.h2
            ...
            className={`${archivo.className} text-4xl lg:text-[64px] lg:leading-[64px] mb-5 lg:mb-8`}
          >
          ...
          <motion.p
            ...
            className="text-black/60 text-sm lg:text-base mb-6 lg:mb-8 max-w-[545px] font-Integral CF"
          >
          ...
          >
            <div className="flex flex-col">
              <span className="font-bold text-2xl md:text-xl lg:text-3xl xl:text-[40px] xl:mb-2">
                <AnimatedNumberCounter from={0} to={200} duration={3} />+
              </span>
            ...
            <div className="flex flex-col ml-6 md:ml-0">
              <span className="font-bold text-2xl md:text-xl lg:text-3xl xl:text-[40px] xl:mb-2">
                <AnimatedNumberCounter from={0} to={2000} duration={4} />+
              </span>
              <span className="text-xs xl:text-base text-black/60 text-nowrap">
            ...
            <div className="flex flex-col w-full text-center sm:w-auto sm:text-left mt-3 sm:mt-0 sm:ml-6 md:ml-0">
              <span className="font-bold text-2xl md:text-xl lg:text-3xl xl:text-[40px] xl:mb-2">
                <AnimatedNumberCounter from={0} to={3000} duration={5} />+
              </span>
              <span className="text-xs xl:text-base text-black/60 text-nowrap">
                Happy Customers
              </span>
        ...
        >
          <Image
            ...
            width={104}
            alt="big star"
            className="absolute right-7 xl:right-0 top-12 max-w-[76px] max-h-[76px] lg:max-w-24 animate-spin"
          />
          <Image
            priority
            ...
            width={56}
            alt="small star"
            className="absolute left-7 md:left-0 top-36 sm:top-64 max-w-11 animate-spin"
          />
        </motion.section>
      </div>
    </header>
